...
struct Record *delete_node(struct Record *head){
	if(head == NULL){
	printf("暂无学员，不能执行删除操作\n");
	return NULL;
	}

	struct Record *p = head,*temp;
	char delete_name[20];
	printf("请输入要删除的学员的姓名：\n");
	scanf("%s",delete_name);
	
	//如果只剩一个人 
	if(p->next==NULL&&!strcmp(p->name,delete_name)){
		free(p);
		printf("已删除，现在没有学员了\n");
		return NULL; 
	}
	//两个及以上，p要指向要删除的节点的上一个节点才行，才能挂载上 
	while(strcmp((p->next->name),delete_name)&&p!=NULL) p = p->next;
	if(!p){
		printf("查无此人，请检查是否输入正确\n");
	}else{
		temp = p->next;
		p->next = temp->next;
		free(temp);
